class Scrollable:
    ...
    def on_press(self, key):
        if key == self.scroll_up:
            ...
            self.min_row = min(self.rows - (self.height - self.horizontal_scrollbar_top - self.horizontal_scrollbar_bottom), self.min_row + 1)
        elif key == self.scroll_left:
            self.min_col = max(0, self.min_col - 1)
        elif key == self.scroll_right:
            self.min_col = min(self.cols - (self.width - self.vertical_scrollbar_left - self.vertical_scrollbar_right), self.min_col + 1)
        else:
