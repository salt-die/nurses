...
with ScreenManager() as sm:
    ...
    for i in range(5):
        widget = sm.root.new_widget(i * 5, 0, 4, 15, create_with="ArrayWin", group="moving", border="light", border_color=colors.PURPLE_ON_BLACK)
        widget.colors[0] = colors.RED_ON_BLACK
        widget[:] = "Hello, World!\nI'm a widget!"

    resize_widget = sm.root.new_widget(9, 40, 12, 18, create_with="ArrayWin", color=colors.BLACK_ON_YELLOW, border="curved", border_color=colors.RED_ON_BLACK)
    resize_widget.colors[:, ::2] = colors.YELLOW_ON_BLACK
    resize_widget[:] = "Resize me!"

    scroll_widget = sm.root.new_widget(9, 60, 18, 12, create_with="ArrayWin", border="double", border_color=colors.CYAN_ON_BLACK)
